...
inline void getDate(char *dateBuf)
{
	...
	sprintf(dateBuf, "%d:%d:%d %d:%d:%d", now->tm_year, now->tm_mon, now->tm_mday, now->tm_hour, now->tm_min, now->tm_sec);
}
...
void Logger::setLogConfig(const string path, const string fileName, const int level)
{
	if(isLogSet != true)
	{
		isLogSet = true;
